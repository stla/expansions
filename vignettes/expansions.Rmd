...
```{r}
identical(intAtBaseFib(6, base=3), intAtBase(8, base=3))
```
...
```{r}
# (3,4,7)-ary expansion of 43:
intToAry(43, sizes=c(3,4,7))
1*1 + 2*3 + 3*(3*4)
```

Note that the last integer of the `sizes` vector is not seemingly involved in the development $d_1 \times 1 + d_2 \times 3 + d_3 \times (3 \times 4)$. 
Actually it is: the digits satisfy $d_1 < 3$, $d_2 < 4$ and $d_3 < 7$. 
Consequently the maximal integer allowing a $(3,4,7)$-ary expansion is $\color{red}{2}\times 1 +\color{red}{3}\times 3+\color{red}{6}\times(3\times 4)=83$:
...
```{r}
# Cartesian product {0,1,2}x{0,1,2}:
sapply(0:8, function(x) intToAry(x, sizes=c(3,3)))
t(expand.grid(c(0,1,2),c(0,1,2)))
```
